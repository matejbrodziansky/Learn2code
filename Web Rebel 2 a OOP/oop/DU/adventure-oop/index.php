...
<body class="container">
	...
	<?php
		...
		$adventure->addDude(
			new Dude('Ice King', 'Armed with a magic crown and an icy heart, the Ice King has only one goal: to secure a wife by any means necessary.', 10, 'cnc programovanie', 'hovno')
		);

		$adventure->addDude(
			new Dude('Nelly', 'The very dangerous agresive dog which bite our stupid complexed neighbor, so nevermind, 
			she probably deserved it, maybe it is happen again  ', 10,  'Je to piča', 'alebo kokot ? ',' je to čudo tak oboje')
		);
	...
	<?php
		...
		{
			foreach ( $dude->commentsText as $item )
			{
				echo '<article>';
				echo '	<h4>'. $dude->who .'</h4>';
				echo '	<p>'. $dude->wat.'<p>';
		...
		// 	echo '	<p>'. $dude->wat.'<p>';
		// 	echo '	<p>Comment ['. $dude->commentsText .']<p>';
		// 	echo '	<p>'. $adventure->commentsText().'<p>';
		// 	echo '</article>';
		// }
