<?php
	...
	function can_edit()
	{
		...
		return $what === 'cerealKiller' && $add == 1;  // musi byť string hodnoty a 1
		//http://localhost:8888/learn2code/80/index.php?what=cerealKiller&add=1
		
		//var_dump( 5 == "5")  // da true porovnava iba hodnotu 
		//var_dump( 5 === "5")  // da fase lebo je to čišlo a string 
		echo '<pre>';
		print_r( $_SERVER);
		echo '</pre>';

		// musi byť voices/index.php
		var_dump ( strpos($_SERVER['REQUEST_URI'], 'index.php')); // hladáme či  v server premennej je index 	print_r( $_SERVER);
	...
	//$data = ['one', 'two'];
	//function push_to_array( $data, 'one', 'two')
	//{
